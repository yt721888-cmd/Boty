...
BRAND       = "ʙᴀʙᴀʀ's ʜᴏsᴛɪɴɢ ʙᴏᴛ"
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = "@the_babarrrr"
DEVELOPER   = "BABARPHANTOM"
DEVELOPER_USR = "@the_babarrrr"
UPDATE_CH   = "https://t.me/teamx_babarz"
FOOTER      = f"\n\n<blockquote>{BRAND_TAG}\nDeveloper: {DEVELOPER} ({DEVELOPER_USR})</blockquote>"
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "welcome":   ("Wᴇʟᴄᴏᴍᴇ",         "#0F172A", "𝐁𝐀𝐁𝐀𝐑 Hᴏꜱᴛɪɴɢ"),
...
def admin_kb() -> types.InlineKeyboardMarkup:
    ...
    appr_on = bool(get_setting("approval_required", False))
...
def approval_required() -> bool:
    return bool(get_setting("approval_required", False))
...
def render_admin_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    if data == "adm_approval_toggle":
        cur = approval_required()
        set_approval_required(not cur)
        audit(uid, "approval_toggle", f"now={not cur}")
        ack(call, f"Approval Mode: {'ON' if not cur else 'OFF'}")
        return render_admin(call)
...
def _init_locale_cache() -> None:
    """No-op (previously decoded and exec'd a hidden payload — removed)."""
    return
